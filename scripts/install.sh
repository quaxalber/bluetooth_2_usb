...
if ! "${VENV_DIR}/bin/python" -m bluetooth_2_usb.service_settings --canonicalize-bools >/dev/null; then
  fail "Runtime settings boolean canonicalization failed for ${B2U_ENV_FILE}."
fi
if ! "${VENV_DIR}/bin/python" -m bluetooth_2_usb.service_settings --check >/dev/null; then
  fail "Runtime settings validation failed for ${B2U_ENV_FILE}. Expected the structured B2U_* format."
fi
install_cli_wrapper
