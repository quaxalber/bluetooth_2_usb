...
from .commands import fail, run, warn
...
def loopback_inject(argv: list[str]) -> int:
    ...
    os.execv(
        ...
        [str(PATHS.venv_python), "-m", "bluetooth_2_usb.loopback", "inject", *argv],
    )
    return 127
...
def loopback_capture(repo_root: Path, argv: list[str]) -> int:
    ...
    os.execve(python_bin, [python_bin, "-m", "bluetooth_2_usb.loopback", "capture", *argv], env)
    return 127
...
def _host_capture_python(repo_root: Path) -> str:
    ...
    if override:
        return override
    candidates = [repo_root / "venv/bin/python", Path("python3"), Path("python")]
    for candidate in candidates:
        command = [str(candidate), "-c", "import hid"]
        if run(command, check=False, capture=True).returncode == 0:
            return str(candidate)
    fail(
        "No suitable Python with hidapi found. Set HOST_CAPTURE_PYTHON or install the Python package 'hidapi'."
    )
