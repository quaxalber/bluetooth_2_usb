...
def run_windows_raw_input_capture(
    ...
    if unsupported_mouse_buttons:
        unsupported_names = [EVENT_CODE_NAMES[EV_KEY].get(code, str(code)) for code in unsupported_mouse_buttons]
        return LoopbackResult(
            command="capture",
            scenario=scenario.name,
            success=False,
            ...
            details={"capture_backend": "raw_input", "unsupported_mouse_buttons": unsupported_names},
        )
