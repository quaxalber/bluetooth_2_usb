...
BUILTINS = "builtins"
VERSION_MODULE = "bluetooth_2_usb.version"
...
class VersionTest(unittest.TestCase):
    def test_package_import_does_not_eagerly_import_args(self) -> None:
        ...
        with patch(f"{BUILTINS}.__import__", side_effect=guarded_import):
            importlib.reload(bluetooth_2_usb)

    def test_prefers_installed_package_metadata(self) -> None:
        with (
            patch(f"{VERSION_MODULE}.package_version", return_value="1.2.3"),
            patch(f"{VERSION_MODULE}.SCM_VERSION", "9.9.9"),
        ):
            self.assertEqual(version.get_version(), "1.2.3")

    def test_falls_back_to_scm_version(self) -> None:
        with (
            patch(f"{VERSION_MODULE}.package_version", side_effect=version.PackageNotFoundError),
            patch(f"{VERSION_MODULE}.SCM_VERSION", "2.0.0.dev1"),
        ):
